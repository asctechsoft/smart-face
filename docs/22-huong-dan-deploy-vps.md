...
| [§12](#12-sơ-đồ-triển-khai-đích-theo-v21) | Topology đích với 6 phân hệ — **chưa thi công** |
...
| File | Làm gì |
|---|---|
| `server-backend-smart/docker-compose.prod.yml` | Dựng 4 service trên VPS: `postgres`, `redis`, `api`, `worker`. Không expose Postgres/Redis ra host. Ảnh và file báo cáo nằm trên Cloud Storage for Firebase, không có service lưu trữ chạy trên VPS |
...
│      │    │    │               │    │                     │
│  ┌───▼────▼────▼───────────────▼────▼──┐                  │
│  │ postgres:5432        redis:6379      │  không ra host   │
│  └──────────────────────────────────────┘                 │
│                                                           │
│   ảnh + file export ──► Cloud Storage for Firebase (ngoài)│
│                                                           │
...
| 2 | **Frontend** | `web-smart` không nằm trong compose và chưa có Dockerfile. Chưa có gì phục vụ giao diện web. Theo v2.1 cần **ba web** (Kế toán, Giám đốc, Platform Admin) — xem [§12](#12-sơ-đồ-triển-khai-đích-theo-v21) | [§5](#5-deploy-frontend-web-smart) |
...
| # | Chốt | Điều kiện chết |
|---|---|---|
| 4 | Biến bắt buộc | Thiếu `AI_SERVER_URL`, `AI_SERVER_INTERNAL_KEY`, `FIREBASE_PROJECT_ID`, `FIREBASE_CLIENT_EMAIL`, `FIREBASE_PRIVATE_KEY` — ba biến `FIREBASE_*` phục vụ CẢ đăng nhập lẫn lưu ảnh |
...
| 9 | Storage | `FIREBASE_STORAGE_EMULATOR_HOST` được đặt — emulator mất sạch file khi tắt |
...
# --- Ứng dụng (tầng 2) -----------------------------------------------------
# NODE_ENV, PORT, DATABASE_URL, REDIS_HOST/PORT, WORKER_ENABLED
# KHÔNG cần khai — compose đã đặt trong `environment:` và ghi đè file này.

# --- Lưu trữ ảnh + file báo cáo (Cloud Storage for Firebase) ---------------
# Không có khoá riêng: dùng chung service account với FIREBASE_* ở dưới.
# Bỏ trống thì lấy bucket mặc định <FIREBASE_PROJECT_ID>.firebasestorage.app;
# dự án Firebase tạo trước 10/2024 dùng đuôi .appspot.com nên phải khai rõ.
...
# 5. Postgres/Redis KHÔNG lộ ra ngoài — cả hai phải fail
...
```yaml
  ai-server:
    ...
    # KHÔNG có `ports:` — docs/11 mục 6.2 cấm expose AI Server ra internet.
    mem_limit: 4g
```
...
### 7.5 Bucket Firebase — hai việc phải làm trước khi chạy thật

Ảnh chấm công, ảnh hồ sơ khuôn mặt và file báo cáo nằm trên Cloud Storage for
Firebase, không đi qua nginx. Link tải là **signed URL V4 do Backend ký tại chỗ**
bằng `FIREBASE_PRIVATE_KEY`, trỏ thẳng tới `storage.googleapis.com` — nên không
cần thêm record DNS hay `server` block nào cho nó.

Đổi lại có hai chốt nằm ngoài repo, phải tự đặt trên Firebase Console:

1. **Khoá Rules** — Firebase Console → Storage → Rules:
   ```
   rules_version = '2';
   ...
   ```
   Backend dùng service account nên **không** đi qua Rules; khoá sạch không làm
   hỏng luồng nào. Để mở là ảnh khuôn mặt nhân viên đọc được từ ngoài — vi phạm
   `NFR-SEC-12`.

2. **Đặt lifecycle rule** làm trần cứng cho chính sách lưu trữ (`attendance/` tối
   đa 400 ngày, `exports/` 30 ngày) — xem
   `server-backend-smart/docs/storage-lifecycle.md`.

> ⚠ Signed URL ký được là nhờ có khoá riêng trong `.env`. Nếu sau này chuyển sang
> chạy bằng Application Default Credentials (Cloud Run, GKE), service account
> phải có quyền `iam.serviceAccounts.signBlob`, nếu không `getPresignedUrl` trả
> `null` và ảnh không hiện — **im lặng**, chỉ có một dòng warn trong log.
...
# Ảnh chấm công + file báo cáo nằm trên Cloud Storage for Firebase, Google đã
# nhân bản sẵn nên KHÔNG cần backup thủ công. Muốn giữ thêm một bản ngoài Google:
#   gcloud storage rsync -r "gs://$FIREBASE_STORAGE_BUCKET" /opt/backups/storage
```
...
| `docker compose` báo `POSTGRES_PASSWORD bắt buộc trong .env` | Chốt tầng 1 — cú pháp `${BIEN:?...}` | Điền `POSTGRES_PASSWORD`, `REDIS_PASSWORD` |
...
| **Ảnh chấm công không hiển thị, `photoUrl` là `null`** | Không ký được signed URL: sai `FIREBASE_PRIVATE_KEY` hoặc bucket không tồn tại. Tìm dòng warn `Không tạo được signed URL` trong `dc logs api` | [§7.5](#75-bucket-firebase--hai-việc-phải-làm-trước-khi-chạy-thật) |
...
| `server-backend-smart/docs/storage-lifecycle.md` | Quy tắc vòng đời từng prefix trên bucket Firebase |
| [11-kien-truc-va-technology-stack.md](./11-kien-truc-va-technology-stack.md) mục 12 | Định hướng môi trường, tách pod API/worker, CI/CD |
| [10-yeu-cau-phi-chuc-nang.md](./10-yeu-cau-phi-chuc-nang.md) | Checklist go-live, các mã `NFR-*` nhắc tới ở đây |
| [09-anti-fraud.md](./09-anti-fraud.md) | `AF-02b` (chốt IP), `AF-12` (chữ ký HMAC), `AF-13` (rate limit) |
| [21-tai-khoan-test.md](./21-tai-khoan-test.md) | Tài khoản seed, dùng để kiểm chứng sau khi deploy |

---

## 12. Sơ đồ triển khai đích theo v2.1

> ⚠ **Phần này CHƯA thi công.** Toàn bộ hướng dẫn từ §0 đến §11 mô tả hệ thống
> **đang chạy được**. Mục này mô tả nơi cần đến theo chuẩn v2.1, để khi mở rộng
> hạ tầng không phải thiết kế lại từ đầu.
...
```
        ...
        │           ┌────────┘  │  └────────┐            │
        ▼           ▼           ▼           ▼            ▼
  ketoan.<domain>  gd.<domain>  admin.<domain>   /v1 → api:3000
  ┌───────────┐  ┌───────────┐  ┌────────────┐  ┌──────────────┐
  │ Web       │  │ Web       │  │ Web        │  │ api          │
  │ Kế toán   │  │ Giám đốc  │  │ Platform   │  │ (NestJS)     │
  │ (SPA)     │  │ (SPA)     │  │ Admin (SPA)│  └──┬───┬───┬───┘
  └───────────┘  └───────────┘  └────────────┘     │   │   │
                                                    │   │   │
   ...
   Nhân viên · Quản lý · Giám đốc                       │   │
                          ...
                          ┌─────────▼───▼──────────────┐
                          │ postgres · redis           │  không ra host
                          └────────────────────────────┘
```

### 12.2. Ba SPA hay một SPA

| Phương án | Ưu | Nhược | Khuyến nghị |
|---|---|---|---|
| **Một SPA, ẩn/hiện theo role** (hiện tại) | Deploy đơn giản, chung component | Bundle của Kế toán vẫn chứa mã màn Platform Admin; ranh giới phân hệ chỉ là quy ước | Tạm chấp nhận cho Kế toán + Giám đốc |
| **Tách Platform Admin ra domain riêng** | Giảm bề mặt tấn công cho tài khoản quyền cao nhất | Thêm một pipeline build | ✅ **Nên làm** — repo đã có sẵn `admin-smart` |
| **Tách cả ba** | Ranh giới rõ nhất | Ba pipeline, ba domain, chi phí vận hành cao hơn | Chỉ khi có yêu cầu bảo mật riêng |

**Kết luận thực dụng:** giữ `web-smart` phục vụ **Web Kế toán + Web Giám đốc**
(tách route và permission theo [05](./05-nghiep-vu-web-ke-toan.md) /
[06](./06-nghiep-vu-web-giam-doc.md)), và **tách `admin-smart` thành domain
riêng**. Đây là điểm cân bằng giữa `A4`/`A5` trong
[07](./07-nghiep-vu-web-platform-admin.md) và chi phí vận hành.

### 12.3. Việc hạ tầng phát sinh thêm

| # | Hạng mục | Vì sao cần | Tài liệu |
|---|---|---|---|
| 1 | Vhost riêng cho `admin-smart` | MFA bắt buộc + IP policy cho Platform Admin (`A4`, `A5`) | [07](./07-nghiep-vu-web-platform-admin.md) |
| 2 | Push notification (FCM) chạy thật | App Giám đốc phụ thuộc push cho cảnh báo khẩn | [04 mục 10](./04-nghiep-vu-app-giam-doc.md) |
| 3 | Correlation ID xuyên service | Truy vết một lượt chấm công qua nginx → api → ai-server → worker | [07 mục 9](./07-nghiep-vu-web-platform-admin.md#9-log-audit--truy-vết-fr-adm-log) |
| 4 | Restore test định kỳ | `FR-ADM-DR-02` — backup chưa restore thử không phải là backup | [07 mục 10](./07-nghiep-vu-web-platform-admin.md) |
| 5 | Retention job cho ảnh chấm công | Chính sách lưu trữ theo tenant/gói | [10 mục 11](./10-yeu-cau-phi-chuc-nang.md) |

> **Không cần Kubernetes để đạt chuẩn v2.1.** Docker Compose + nginx đủ cho quy
> mô một vài tenant. Điều bắt buộc là **service boundary và observability đúng
> ngay từ đầu** — xem [11 mục 4.1](./11-kien-truc-va-technology-stack.md).
