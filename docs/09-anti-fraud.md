# 09 — Chống gian lận chấm công (Anti-Fraud)

> **Nguồn chuẩn:** Tài liệu nghiệp vụ SmartFace **v2.1**, Chương IX.
...
Với vân tay, cơ chế tương đương là **challenge–response có chữ ký từ secure enclave** (xem `02-nghiep-vu-app-nhan-vien.md` mục 4.2) — Backend verify chữ ký bằng public key đã lưu, không nhận cờ boolean.
...
Công thức hash: xem [08 mục 1.2.1](./15-hop-dong-api.md#121-x-body-sha256--ràng-buộc-nội-dung-của-request-multipart).
...
## 12. Dashboard nghi vấn & quy trình quyết định (v2.1 §9.6)

Bảng này chuẩn hoá **ai xem gì trước khi quyết định** với từng loại cờ nghi vấn.
Không có bảng này thì mỗi người xử lý một kiểu và không đối soát được về sau.

| Risk flag | Dữ liệu cần xem | Quyết định cho phép |
|---|---|---|
| **Mock GPS** | GPS/provider/accuracy, Wi-Fi/BLE, device attestation | Giữ công · Huỷ · Yêu cầu xác minh |
| **Liveness thấp** | AI score, model version, ảnh (theo quyền) | Chấp nhận · Đăng ký lại face · Điều tra |
| **Thiết bị lạ** | Device ID, session, app integrity | Revoke session · Reset device |
| **Impossible travel** | Hai location + timestamp | Gắn nghi vấn · Kiểm tra đơn công tác |
| **Attendance bất thường** | Giờ quá ngắn/dài, ca, đơn liên quan | Điều chỉnh qua workflow |

### 12.1. Ai được làm gì

| Vai trò | Quyền với cờ nghi vấn |
|---|---|
| **Nhân viên** | Thấy lượt chấm của mình bị gắn cờ và trạng thái đối soát |
| **Quản lý** | **Chỉ xem** ở mức thông tin, để nhắc nhân viên xử lý sớm |
| **Kế toán** | Quyết định *Chấp nhận / Huỷ công / Chờ xác minh* — **bắt buộc có lý do** |
| **Giám đốc** | Xem tổng hợp và xu hướng; quyết định các trường hợp cần override |
| **Platform Admin** | **Không** quyết định nghiệp vụ. Chỉ hỗ trợ kỹ thuật qua Support Access Mode |

> **Không hiển thị raw biometric theo mặc định** ở bất kỳ màn hình quản trị nào.
> Truy cập ảnh/embedding phải theo chính sách dữ liệu, có masking và ghi audit
> (`A6` trong [07](./07-nghiep-vu-web-platform-admin.md)).

### 12.2. Nguyên tắc nền của chương IX

> **Không tin dữ liệu do client tự khai.** Mọi yếu tố quyết định công phải được
> Backend / AI / attestation kiểm chứng và có log (`BR-02`).

Năm kịch bản trong tài liệu này (mục 2–6) tương ứng §9.1–§9.5 của v2.1. Phần
`AF-01`…`AF-23` là bản chi tiết hoá các biện pháp, giữ nguyên hiệu lực.

---

**Tiếp theo:** [10 — Yêu cầu phi chức năng](./10-yeu-cau-phi-chuc-nang.md)
