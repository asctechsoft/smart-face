# 02 — Nghiệp vụ App Nhân viên

> **Nguồn chuẩn:** Tài liệu nghiệp vụ SmartFace **v2.1**, Chương II.
> Nền tảng: **Flutter (Dart)** — **một codebase chung** cho App Nhân viên,
> [App Quản lý](./03-nghiep-vu-app-quan-ly.md) và
> [App Giám đốc](./04-nghiep-vu-app-giam-doc.md); module mở theo
> `role + permission + scope`.
> Actor: `ACT-EMP` (Nhân viên).

App Nhân viên là kênh thao tác cá nhân hằng ngày. Mục tiêu: **giảm số bước khi
chấm công**, làm rõ trạng thái công, và giúp nhân viên tự kiểm tra dữ liệu
**trước khi kỳ công được chốt**.
...
> [08 mục 2](./15-hop-dong-api.md#2-api-xác-thực-auth) ·
> [13](./19-luong-onboarding-va-dang-ky-khuon-mat.md)
>
...
9. [Bảng công tuần/tháng/năm](#9-bảng-công-tuần--tháng--năm-fr-app-tsheet)
10. [Lịch làm việc](#10-lịch-làm-việc-fr-app-cal)
11. [Thông báo](#11-thông-báo-fr-app-noti)
12. [Thống kê & tính năng mở rộng](#12-thống-kê--tính-năng-mở-rộng-fr-app-stat)
13. [Kiến trúc App](#13-kiến-trúc-app-flutter)
14. [Ngoại lệ & tiêu chí nghiệm thu](#14-ngoại-lệ--tiêu-chí-nghiệm-thu-app-nhân-viên)
...
## 9. Bảng công tuần / tháng / năm (`FR-APP-TSHEET`)

> **Mới theo v2.1 §2.7.** Bản v1.0 chỉ có "Lịch sử" (raw event). v2.1 tách rõ
> **lượt chấm công** khỏi **bảng công**.

> ### Phân biệt hai khái niệm
>
> **Lượt chấm công (raw event)** — mục 7 — là sự kiện thô: ai, lúc nào, ở đâu,
> bằng phương thức gì. Bất biến (`BR-06`).
>
> **Bảng công (timesheet)** — mục này — là **kết quả đã tính**: tổng hợp từ lượt
> chấm + lịch ca + đơn đã duyệt + chính sách có hiệu lực. Tính lại được bất cứ
> lúc nào từ dữ liệu thô.
>
> Nhân viên khiếu nại "sao tháng này thiếu công" thì phải xem được **cả hai** để
> hiểu số liệu đến từ đâu.

| Chế độ | Thông tin hiển thị |
|---|---|
| **Tuần** | 7 ngày, ca từng ngày, giờ làm, thiếu/dư, đơn liên quan. Phù hợp kiểm tra nhanh trước cuối tuần |
| **Tháng** | Công chuẩn, công thực tế, ngày phép, không lương, OT, phút trễ/sớm, thiếu công, ngày lễ; chi tiết theo ngày |
| **Năm** | Tổng hợp 12 tháng: công, phép đã dùng/còn lại, OT, số lần vi phạm; drill-down về tháng |

| Mã | Yêu cầu | Ưu tiên |
|---|---|---|
| `FR-APP-TSHEET-01` | Xem bảng công tháng: công chuẩn, thực tế, phép, OT, trễ/sớm, thiếu công | Must |
| `FR-APP-TSHEET-02` | Xem bảng công tuần với chi tiết từng ngày và đơn liên quan | Must |
| `FR-APP-TSHEET-03` | Xem tổng hợp năm, drill-down về tháng | Should (GĐ 2) |
| `FR-APP-TSHEET-04` | Kỳ công `LOCKED` → hiển thị nhãn **"Đã chốt"**; yêu cầu điều chỉnh mới đi theo luồng hậu kiểm (`BR-07`) | Must |
| `FR-APP-TSHEET-05` | Từ một ngày bất thường, mở thẳng được lượt chấm gốc và đơn liên quan | Must |
| `FR-APP-TSHEET-06` | Tải/xem phiếu công cá nhân dạng PDF | Could (GĐ 2) |

> **Số liệu năm không thay thế bảng lương.** Đây chỉ là tổng hợp chuyên cần và
> công theo dữ liệu đã chốt. Nếu sau này có module Payroll đầy đủ, hai con số
> này phải được trình bày tách bạch để nhân viên không hiểu nhầm.

---

## 10. Lịch làm việc (`FR-APP-CAL`)

> **Mới theo v2.1 §2.8.**

| Mã | Yêu cầu | Ưu tiên |
|---|---|---|
| `FR-APP-CAL-01` | Lịch tháng/tuần hiển thị ca được phân, ngày nghỉ, ngày lễ, công tác, đổi ca đã duyệt | Must |
| `FR-APP-CAL-02` | Nhấn vào ngày xem chi tiết ca: giờ bắt đầu/kết thúc, nghỉ giữa ca, văn phòng áp dụng, yêu cầu chấm công | Must |
| `FR-APP-CAL-03` | Khi Giám đốc/Quản lý thay đổi phân ca → App nhận push và hiển thị lịch mới | Must |
| `FR-APP-CAL-04` | **Lưu lịch sử thay đổi ca** để giải quyết tranh chấp: ai đổi, lúc nào, từ ca nào sang ca nào | Must |

> `FR-APP-CAL-04` không phải tính năng phụ. Khi nhân viên nói "tôi không được
> báo là đổi ca", lịch sử thay đổi là bằng chứng duy nhất.

---

## 11. Thông báo (`FR-APP-NOTI`)

> **Mới theo v2.1 §2.9.**

| Nhóm thông báo | Ví dụ |
|---|---|
| **Chấm công** | Sắp tới giờ vào ca nhưng chưa chấm; đã vào nhưng chưa chấm ra; lượt chấm bị nghi vấn |
| **Đơn từ** | Đơn được duyệt/từ chối/yêu cầu bổ sung; đơn sắp tới ngày hiệu lực |
| **Lịch ca** | Ca mới, đổi ca, thay đổi giờ làm |
| **Bảng công** | Sắp chốt kỳ, còn thiếu công; kỳ đã chốt |
| **Công ty** | Thông báo nghỉ lễ, bảo trì, chính sách mới |

| Mã | Yêu cầu | Ưu tiên |
|---|---|---|
| `FR-APP-NOTI-01` | Nhận push theo 5 nhóm trên | Must |
| `FR-APP-NOTI-02` | Notification center xem lại thông báo cũ, đánh dấu đã đọc | Must |
| `FR-APP-NOTI-03` | Cấu hình bật/tắt từng nhóm, **trừ cảnh báo bảo mật bắt buộc** | Should |
| `FR-APP-NOTI-04` | Deep link **phải kiểm tra lại quyền khi mở** — không dựa vào việc đã nhận notification để cho truy cập | Must |
| `FR-APP-NOTI-05` | Thông báo cũ hiển thị **trạng thái mới nhất** của đối tượng, tránh thao tác trên dữ liệu đã thay đổi | Must |

---

## 12. Thống kê & tính năng mở rộng (`FR-APP-STAT`)
...
### 12.1. Lưu ý thi công chế độ offline (`FR-APP-STAT-06`)
...
### 13.2. Thư viện chính (theo PA 7.2)
...
## 14. Ngoại lệ & tiêu chí nghiệm thu App Nhân viên
...
|---|---|
| **Mất mạng lúc chấm** | Nếu tenant bật offline: lưu gói chấm đã ký cục bộ, đồng bộ khi có mạng, đánh dấu `OFFLINE` và kiểm tra chống sửa timestamp. Nếu không bật: **báo rõ không thể chấm** (xem mục 12.1) |
| **GPS yếu** | Không chấp nhận nếu `accuracy` vượt ngưỡng cấu hình; hướng dẫn bật GPS / ra vị trí thoáng |
| **Ngoài geofence** | Chặn, hoặc cho chấm nhưng gắn cờ — tuỳ chính sách tenant và đơn công tác đã duyệt |
| **Không có ca** | Theo chính sách: cho chấm ca linh hoạt, hoặc chặn và yêu cầu liên hệ quản lý |
| **Đổi thiết bị** | Buộc đăng ký lại bảo mật; thiết bị cũ bị vô hiệu hoặc chuyển trạng thái (`BR-11`) |
| **Kỳ công đã khoá** | **Không sửa trực tiếp**; tạo yêu cầu điều chỉnh hậu kiểm (`BR-07`) |
| **Bấm chấm công nhiều lần** | Backend dùng nonce/idempotency key — không tạo bản ghi trùng (`BR-19`) |
| **Ca qua đêm** | Attendance gắn theo **ngày ca**, không theo ngày lịch (`BR-20`) |

### 14.2. Bảng trạng thái ngày công

Đây là các trạng thái App phải hiển thị được trên Trang chủ, Lịch sử và Bảng công:

| Trạng thái | Ý nghĩa | Hiển thị gợi ý |
|---|---|---|
| **Đúng giờ** | Đủ điều kiện ca và giờ | Màu bình thường, đủ công |
| **Đi muộn** | Check-in sau ngưỡng cho phép | Hiển thị số phút muộn |
| **Về sớm** | Check-out trước ngưỡng | Hiển thị số phút thiếu |
| **Thiếu chấm vào/ra** | Chỉ có một đầu chấm | Cảnh báo + nút tạo đơn bổ sung công |
| **Nghỉ có phép** | Đơn nghỉ đã duyệt | Không yêu cầu chấm công trong thời gian nghỉ |
| **Công tác** | Đơn công tác đã duyệt | Có thể bỏ geofence theo chính sách |
| **OT** | Có thời lượng vượt ca được công nhận | Hiển thị giờ OT dự kiến/đã duyệt |
| **Nghi vấn** | Có cờ anti-fraud | Nhân viên **vẫn thấy bản ghi**, nhưng trạng thái có thể chờ đối soát |

> Nhân viên phải thấy được lượt chấm bị gắn cờ nghi vấn. Giấu đi thì đến cuối
> tháng họ mới phát hiện thiếu công và không còn bằng chứng để khiếu nại.

### 14.3. Mã lỗi nghiệp vụ sinh trắc học

| Mã lỗi | Tình huống | Hướng xử lý trên App |
|---|---|---|
| `FACE_NO_FACE` | Không phát hiện khuôn mặt | Nhắc đưa mặt vào khung, thử lại |
| `FACE_MULTI_FACE` | Có nhiều khuôn mặt | Chỉ cho phép một người trong khung |
| `FACE_LOW_QUALITY` | Ảnh mờ/tối/ngược sáng | Hướng dẫn chỉnh ánh sáng/khoảng cách |
| `LIVENESS_FAILED` | Nghi giả mạo | Chặn lượt thử, ghi log, tăng thời gian cooldown |
| `BIOMETRIC_UNAVAILABLE` | Máy không hỗ trợ sinh trắc học | Chuyển sang khuôn mặt hoặc phương án công ty cho phép |
| `DEVICE_CHANGED` | Thiết bị thay đổi | Yêu cầu xác thực lại và đăng ký thiết bị mới |

Hợp đồng đầy đủ: [15 mục 3](./15-hop-dong-api.md#3-api-sinh-trắc-học-biometric).

### 14.4. Tiêu chí nghiệm thu (UAT)

- [ ] Chỉ ghi nhận lượt chấm khi **đủ điều kiện policy**; thời gian tính từ server (`BR-01`).
- [ ] Retry request chấm công **không tạo bản ghi trùng** (`BR-19`).
- [ ] Không chấp nhận ảnh giả cơ bản (ảnh in, ảnh trên màn hình).
- [ ] Mọi mã lỗi sinh trắc học đều có thông báo và hướng dẫn cụ thể, không phải "Đã có lỗi xảy ra".
- [ ] Reset sinh trắc học / đổi thiết bị có luồng rõ ràng, không để tài khoản kẹt.
- [ ] Ca qua đêm quy đúng ngày công, không tách sai ở mốc 00:00 (`BR-20`).
- [ ] Bảng công và lịch sử đọc cùng một nguồn — tổng tháng khớp tổng các ngày.
- [ ] Kỳ đã chốt hiển thị nhãn "Đã chốt" và chặn mọi thao tác sửa trực tiếp.

---

**Tiếp theo:** [03 — Nghiệp vụ App Quản lý](./03-nghiep-vu-app-quan-ly.md)
