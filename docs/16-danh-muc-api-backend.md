# 16 — Danh mục API Backend (thực tế đang có trong mã nguồn)

> **Nguồn:** đọc trực tiếp từ `server-backend-smart/src/modules/**/*.controller.ts` tại thời điểm 09/08/2026.
> Tài liệu này mô tả **API đã thi công**, khác với [15-hop-dong-api.md](./15-hop-dong-api.md) là **hợp đồng thiết kế**.
...
> **Phải hỏi lại mỗi khi đổi loại đơn hoặc khoảng ngày.** Số bước duyệt phụ thuộc **độ dài đơn**: nghỉ 1 ngày chỉ cần trưởng phòng, từ 3 ngày trở lên mới thêm bước HR (`ApprovalFlowStep.condition`, docs/05 mục 4.1). Người nhập hộ không đoán được điều đó.
>
...
`employeeIds` (tuỳ chọn) thu hẹp xuống vài người — nút "Xuất chi tiết" ở màn chi tiết bảng công của một CBNV dùng nó. Xuất cả phòng ban khi người dùng đang xem đúng một người là trả về một file họ phải tự lọc lại, và với công ty 500 người thì đó là một file không mở nổi.

⚠ Nó chỉ **thu hẹp**, không bao giờ nới. Phạm vi phòng ban vẫn được chốt lúc nhận yêu cầu rồi ghi vào params của job (`BR-09` — worker chạy ở pod khác, sau khi request đã kết thúc, nên không suy lại được quyền), và worker lấy GIAO hai tập trước khi đọc dữ liệu: một `MANAGER` gửi kèm id của người ngoài phòng mình vẫn không lấy được dòng nào của người đó. Luật này là hàm thuần `resolveExportEmployeeFilter`, khoá bằng `export-scope.spec.ts`.
...
#### `GET /v1/admin/attendance/adjustments` — Lịch sử hiệu chỉnh của một CBNV 🔒

**Làm gì:** trả về mọi lượt hiệu chỉnh công của một nhân viên trong khoảng ngày, mới nhất trước. `BR-ADJ-01` quy định hiệu chỉnh **không sửa đè** bản ghi thô mà tạo `attendance_adjustment` riêng, nên đây là cách duy nhất đọc lại "ai đã sửa gì, vì sao".

⚠ **Đừng nhầm với `GET /v1/attendance/adjustments`** (không có `admin/`). Đường kia là endpoint TỰ PHỤC VỤ: nó lấy `employeeId` từ JWT của người đang đăng nhập và **bỏ qua** tham số gửi lên. Gọi nhầm nó từ màn quản trị sẽ hiện lịch sử điều chỉnh của chính kế toán đang mở màn hình, dưới tên nhân viên họ đang xem.

`MANAGER` chỉ đọc được CBNV thuộc phòng ban mình quản lý; ngoài phạm vi trả `EMP_NOT_FOUND` chứ không `AUTH_FORBIDDEN` — nói "bạn không được xem người này" là đã xác nhận người này tồn tại.

```bash
curl "http://localhost:3000/v1/admin/attendance/adjustments?employeeId=emp_01J...&from=2026-08-01&to=2026-08-31" \
  -H "Authorization: Bearer $HR_TOKEN"
```
```json
{ "success": true,
  "data": [{ "id": "adj_01J...", "employeeId": "emp_01J...",
             "workDate": "2026-08-07", "attendanceLogId": null,
             "adjustType": "ADD",
             "beforeValue": null,
             "afterValue": { "lastCheckOutAt": "2026-08-07T10:30:00Z" },
             "reason": "Nhân viên quên chấm ra, có xác nhận của quản lý",
             "requestId": null,
             "createdByUserId": "usr_01J...",
             "createdByName": "Lê Thị Hoa",
             "createdAt": "2026-08-08T02:14:00Z" }] }
```

`workDate` là ngày công BỊ SỬA, khác `createdAt` là lúc thao tác — sửa hôm nay cho ngày hôm kia là trường hợp thường gặp nhất. `createdByName` do Backend tra sẵn từ `user_account`: bảng `attendance_adjustment` không có quan hệ đó, và một chuỗi `cmsikw…` trên màn hình thì không trả lời được câu hỏi duy nhất mà lịch sử điều chỉnh sinh ra để trả lời.

Lỗi: `EMP_NOT_FOUND` (404).

#### `GET /v1/admin/attendance-sheets/summary` — Tổng hợp công cả THÁNG 🔒

**Làm gì:** cửa vào của màn "Bảng công". Gộp **mọi bảng chấm công của một tháng** thành một bảng phẳng — mỗi dòng một người.

⚠ Route này phải khai TRƯỚC `@Get(':id')` trong controller. Nest so khớp theo thứ tự khai báo, để sau thì `/summary` bị hiểu thành một `id` và luôn trả `ATT_SHEET_NOT_FOUND`.

**Vì sao trục là tháng, không phải từng bảng.** Một tháng được chia thành nhiều bảng theo nhóm phòng ban. Đó là đơn vị TỔ CHỨC — nó chốt xem ai thuộc kỳ, và nó là thứ được chốt — nhưng không phải đơn vị người dùng nghĩ tới: kế toán hỏi *"tháng 5 còn ai chưa xong"*, không hỏi *"bảng tháng 5 của Kho vận còn ai chưa xong"*. Các bảng vẫn trả về trong `sheets`.

`month` nhận ngày bất kỳ trong tháng và tự chuẩn hoá về ngày 01 — kỳ của bảng luôn neo ở đó. Bỏ trống = tháng hiện tại. Tháng chưa lập bảng nào trả **khung rỗng**, không phải 404: màn hình này phải mở được từ ngày đầu tiên.

`totals` gộp trên **toàn bộ** người khớp bộ lọc, không phải trên trang đang mở — hàng thẻ chỉ số nói về cả tháng, và nó không được nhảy mỗi lần lật trang.

```bash
curl "http://localhost:3000/v1/admin/attendance-sheets/summary?month=2026-08-01&departmentId=dep_01J...&pageSize=20" \
  -H "Authorization: Bearer $HR_TOKEN"
```
```json
{ "success": true,
  "data": { "period": { "month": "2026-08-01", "from": "2026-08-01", "to": "2026-08-31" },
            "sheets": [{ "id": "ash_01J...", "name": "Bảng chấm công Tháng 08/2026",
                         "status": "DRAFT", "departmentIds": ["dep_01J..."],
                         "memberCount": 42, "needsReviewCount": 6 }],
            "lastCalculatedAt": "2026-08-17T09:33:51.943Z",
            "totals": { "employeeCount": 256, "standardDays": 5632, "actualDays": 5418.5,
                        "otMinutes": 18765, "needsReviewCount": 24 },
            "rows": [{ "employeeId": "emp_01J...", "employeeCode": "ducnv.amobi",
                       "fullName": "Nguyễn Văn Đức",
                       "department": { "id": "dep_01J...", "name": "Kho" },
                       "sheetId": "ash_01J...",
                       "standardDays": 26, "actualDays": 25.5, "missingDays": 0.5,
                       "otMinutes": 1470, "workedMinutes": 12240,
                       "lateMinutes": 30, "earlyLeaveMinutes": 15,
                       "leaveDays": 1, "status": "NEEDS_REVIEW" }] },
  "meta": { "page": 1, "pageSize": 20, "total": 256 } }
```

Ba cột "công" đến từ **ba nguồn khác nhau**, và đặt chúng cạnh nhau chính là mục đích của bảng:

| Trường | Nguồn | Nghĩa |
|---|---|---|
| `standardDays` | `shift_assignment` | số **ngày** được xếp ca (đếm theo ngày, không theo lượt ca) |
| `actualDays` | `attendance_daily.standard_days` | công engine đã tính |
| `missingDays` | hiệu hai cột trên, kẹp ở 0 | phần chưa giải trình được |

`leaveDays` là số ngày `ON_LEAVE` — **cùng định nghĩa** với `PayrollService.buildSummaries`. Đếm lại từ đơn từ sẽ ra con số khác cho đơn bắc qua hai tháng, và khi đó bảng công với bảng lương nói hai điều khác nhau về cùng một người.

`sheetId` của mỗi dòng là bảng đang giữ người đó. Cần nó vì màn chi tiết của một CBNV nằm DƯỚI bảng (`/attendance/:sheetId/employees/:employeeId`), và vì `status` khoá theo bảng: một tháng chốt **dần** từng bảng, nên người của bảng chưa chốt vẫn sửa được.

`status` của dòng, theo thứ tự ƯU TIÊN: `LOCKED` (bảng giữ người này đã chốt) › `MISSING_CHECK_OUT` › `NEEDS_REVIEW` › `VALID`. Điều kiện "cần đối soát" khớp từng nhánh với `GET /v1/admin/dashboard/reconciliation` — thẻ "Cần đối soát" ở Tổng quan và ở bảng công phải đếm cùng một thứ.

**Hai tập người, hai mục đích.** `needsReviewCount` trong `totals` đếm theo BỘ LỌC đang bật (chi nhánh / phòng ban / tìm kiếm) — nó mô tả cái đang hiện trên màn hình. `needsReviewCount` của từng phần tử `sheets` đếm trên TOÀN bảng, bỏ qua bộ lọc đó — nó nuôi hộp thoại chốt, và lọc "Kế toán" rồi thấy Kho vận ghi 0 người cần rà là dẫn thẳng tới một lần chốt sai.

`employeeId=<id>` thu về **đúng một người** — dùng cho màn "Chi tiết bảng công" của từng CBNV. Khác `q` ở chỗ nó chính xác: `q` khớp theo `contains` nên mã `NV001` còn kéo về cả `NV0011`. Cùng tham số này có ở `/:id/board`.

`needsReviewOnly=true` thu hẹp **dòng** chứ không đụng `totals`: cảnh báo "24 nhân viên cần đối soát" và danh sách 24 người đó phải đếm cùng một tập.
...
>
> ⚠ Hệ số ngày công hiện **chỉ được lưu và hiển thị**. Máy tính công chưa đọc tới, nên sửa chúng không làm đổi số liệu kỳ lương — xem `docs/06` mục 6.6.
...
| GET | `/v1/admin/attendance-sheets/summary` | MGR/HR/ADMIN | Tổng hợp công cả THÁNG — mỗi dòng một người |
| GET | `/v1/admin/attendance/adjustments` | MGR/HR/ADMIN | Lịch sử hiệu chỉnh công của một CBNV (`BR-ADJ-01`) |
...
| PATCH | `/shift-schedules/:id` | MGR/HR/ADMIN 📝 | Sửa tên / phạm vi — **Web Quản lý không gọi** (xem docs/05 mục 8.4) |
...
diễn sẽ sai ở một trong hai. Xem công thức tính công ở docs/05 mục 7.3.
...
## Phụ lục C — API còn thiếu so với chuẩn v2.1

> Tài liệu này liệt kê API **đang có trong mã nguồn**. Phụ lục này liệt kê phần
> **chưa có** mà chuẩn v2.1 yêu cầu, để đội thi công không nhầm giữa "đã làm" và
> "đã đặc tả".

### C.1. App Quản lý — dùng lại được API sẵn có

| Nghiệp vụ v2.1 | Endpoint hiện có | Trạng thái |
|---|---|---|
| Danh sách chờ duyệt | `GET /v1/requests/pending-approval` | ✅ dùng lại |
| Duyệt / từ chối | `POST /v1/requests/:id/approve` · `/reject` | ✅ dùng lại |
| Duyệt hàng loạt | `POST /v1/requests/bulk-approve` | ✅ dùng lại |
| Theo dõi chấm công đội nhóm | `GET /v1/admin/attendance/*` (đã có `ScopeGuard`) | ⚠ cần bản rút gọn cho mobile |
| Yêu cầu bổ sung thông tin | — | ❌ **thiếu** trạng thái `NEED_MORE_INFO` |
| Phân ca được uỷ quyền | `/v1/admin/shift-schedules` | ⚠ cần permission `shift.assign` theo scope |

### C.2. App Giám đốc — cần bổ sung

| Nghiệp vụ | Endpoint đề xuất | Ghi chú |
|---|---|---|
| Dashboard điều hành | `GET /v1/exec/dashboard` | KPI + drill-down, có `scope` param, trả `updatedAt` |
| Trung tâm phê duyệt | `GET /v1/exec/approvals` | Gộp đơn Quản lý + yêu cầu Kế toán + kỳ công |
| Duyệt chốt kỳ | `POST /v1/exec/periods/:id/approve-lock` | Cần `period.approve_lock` + step-up |
| Duyệt mở lại kỳ | `POST /v1/exec/periods/:id/approve-reopen` | Bắt buộc `reason` + step-up |
| Theo dõi chấm công toàn công ty | `GET /v1/exec/attendance/overview` | Theo chi nhánh/phòng ban |
| Báo cáo nhanh | `GET /v1/exec/reports/quick` | Chart-oriented, không export nặng |
| Uỷ quyền tạm thời | `POST /v1/exec/delegations` | `validFrom`/`validTo` + audit |
| Cấp/thu hồi Owner | `POST /v1/exec/owners` · `DELETE /v1/exec/owners/:id` | Chặn mất Owner cuối (`BR-15`) |
| Step-up challenge | `POST /v1/auth/step-up` · `POST /v1/auth/step-up/verify` | Trả token ngắn hạn cho thao tác nhạy cảm |

### C.3. Kỳ công — mở rộng state machine

| Nghiệp vụ | Endpoint hiện có | Cần thêm |
|---|---|---|
| Chốt kỳ | `POST /v1/admin/payroll/periods/:id/lock` | Tách thành **gửi duyệt** (Kế toán) và **duyệt chốt** (Giám đốc) |
| Mở lại kỳ | `POST /v1/admin/payroll/periods/:id/unlock` | Đổi thành **yêu cầu mở lại** → Giám đốc duyệt, bắt buộc `reason` |
| Version kết quả | — | ❌ **thiếu** `GET /v1/admin/payroll/periods/:id/versions` |

### C.4. Platform Admin — cần bổ sung

| Nghiệp vụ | Endpoint đề xuất |
|---|---|
| Gói dịch vụ & feature flags | `/v1/system/packages` · `/v1/system/tenants/:id/features` |
| Incident management | `/v1/system/incidents` |
| Support Access Mode | `POST /v1/system/support-sessions` · `DELETE /v1/system/support-sessions/:id` |
| Truy vết theo correlation ID | `GET /v1/system/trace/:correlationId` |

### C.5. Chung — RBAC + Scope

Mọi endpoint ở trên **và toàn bộ endpoint hiện có** phải kiểm đủ sáu điều kiện ở
[08 mục 2.2](./08-luong-xuyen-phan-he-va-ma-tran-quyen.md#22-quy-tắc-kiểm-tra-bắt-buộc-ở-backend):
`tenant_id` · `permission` · `scope` · trạng thái · version · chống tự duyệt.

---

## Tài liệu liên quan

- [15-hop-dong-api.md](./15-hop-dong-api.md) — hợp đồng API thiết kế (bao gồm cả Backend ↔ AI Server)
- [18-luong-cham-cong-chi-tiet.md](./18-luong-cham-cong-chi-tiet.md) — payload thật ở từng chặng của một lượt chấm công
- [19-luong-onboarding-va-dang-ky-khuon-mat.md](./19-luong-onboarding-va-dang-ky-khuon-mat.md) — luồng onboarding
- [09-anti-fraud.md](./09-anti-fraud.md) — chi tiết 23 biện pháp `AF-01`–`AF-23`
- [13-mo-hinh-du-lieu.md](./13-mo-hinh-du-lieu.md) · [14-so-do-quan-he-bang-du-lieu.md](./14-so-do-quan-he-bang-du-lieu.md) — mô hình dữ liệu
