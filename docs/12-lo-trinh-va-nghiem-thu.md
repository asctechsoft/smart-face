# 12 — Lộ trình triển khai & Tiêu chí nghiệm thu

> **Nguồn chuẩn:** Tài liệu nghiệp vụ SmartFace **v2.1**, Chương XII.
> Mở rộng thành **epic backlog có thể giao việc**. Ước lượng theo **người-tuần
> (person-week)**, giả định đội 5–7 người.
...
## 1. Ba giai đoạn theo v2.1

| Giai đoạn | Mục tiêu | Nội dung |
|---|---|---|
| **GĐ 1 — MVP vận hành được** | Chạy được vòng nghiệp vụ khép kín cho 1 công ty, trên **cả 6 phân hệ** | Phạm vi chi tiết theo phân hệ ở mục 1.1 |
| **GĐ 2 — Hoàn thiện nghiệp vụ doanh nghiệp** | Đáp ứng công ty có nghiệp vụ phức tạp | Đầy đủ loại đơn, approval workflow nhiều cấp, uỷ quyền tạm thời · ca xoay/ca qua đêm nâng cao, phân ca chu kỳ, import ca · phép/OT/công bù nâng cao, version policy, cảnh báo cuối kỳ · báo cáo nâng cao, template export, tích hợp MISA/FAST/API · platform package/feature flag, subscription, tenant usage analytics · dashboard anti-fraud và quy trình review nghi vấn |
| **GĐ 3 — Scale & AI nâng cao** | Mở rộng quy mô và chống gian lận nâng cao | Canary model, model registry, quality monitoring, vector search quy mô lớn · offline attendance an toàn, BLE/Wi-Fi signal, risk scoring đa tín hiệu · multi-region/DR nâng cao, autoscaling AI GPU · **Payroll đầy đủ** nếu mở rộng từ "tính công" sang "tính lương" · SSO doanh nghiệp, SCIM, API/connector với HRM/ERP |

### 1.1. Phạm vi MVP theo từng phân hệ

| Phân hệ | Phạm vi MVP |
|---|---|
| **App Nhân viên** | Đăng nhập, tham gia tenant, đăng ký face/biometric, chấm vào/ra, geofence cơ bản, đơn nghỉ/bổ sung công, lịch sử, bảng công tháng, profile |
| **App Quản lý** | Toàn bộ App Nhân viên **+** duyệt đơn nhân viên **+** trạng thái chấm công đội nhóm |
| **App Giám đốc** | Dashboard toàn công ty, approval center cấp cao, theo dõi chấm công, trạng thái kỳ công, **duyệt chốt/mở lại kỳ**, báo cáo nhanh, notification cảnh báo |
| **Web Kế toán** | Nhân sự, xem chấm công, tính công tháng, xử lý thiếu công, kỳ công, export Excel |
| **Web Giám đốc** | Duyệt đơn cấp cao, tổ chức/phòng ban, giờ làm, mẫu ca, phân ca, luồng duyệt cơ bản, phân quyền, geofence |
| **Web Platform Admin** | Tenant, gói cơ bản, AI health, user support, log, incident cơ bản |
| **Anti-fraud** | Liveness, mock GPS signal, device binding, server timestamp, request anti-replay cơ bản |

> **So với v1.0, MVP nở thêm hai phân hệ mobile** — App Quản lý tách khỏi Web và
> App Giám đốc hoàn toàn mới. Ước lượng ở mục 11 **chưa tính phần này**; phần bổ
> sung nằm ở [mục 12](#12-phần-việc-bổ-sung-do-v21-chưa-có-trong-ước-lượng-mục-11).
...
| `E0.7` Hạ tầng dev | PostgreSQL, Redis, Firebase Emulator (auth + storage), seed data, script khởi tạo tenant demo | 1 tuần | `E0.1` |
...
1. **Chốt các câu hỏi mở** trong `01-tong-quan-he-thong.md` mục 15 (Q1–Q8) — đặc biệt Q3 (duyệt 1 cấp hay nhiều cấp) và Q4 (liveness tự làm hay dùng SDK), vì hai câu này ảnh hưởng kiến trúc.
...
## 12. Phần việc bổ sung do v2.1 (chưa có trong ước lượng mục 11)

| Epic | Nội dung | Ước lượng | Tài liệu |
|---|---|---:|---|
| `E-V21.1` RBAC + Data Scope | Mở rộng `managedDepartmentIds` thành mô hình Role × Permission × Scope 6 mức, `ScopeGuard` dùng chung | 3 tuần | [08 mục 2](./08-luong-xuyen-phan-he-va-ma-tran-quyen.md#2-mô-hình-quyền-role--permission--scope) |
| `E-V21.2` Approval workflow cấu hình được | Workflow versioning + snapshot khi gửi đơn, fallback approver, uỷ quyền tạm thời | 3 tuần | [06 mục 9](./06-nghiep-vu-web-giam-doc.md#9-cấu-hình-luồng-duyệt-fr-gdw-flow) |
| `E-V21.3` State machine kỳ công | Thêm `PENDING_APPROVAL`, `REOPENED`, versioning kết quả tính công | 2.5 tuần | [05 mục 13](./05-nghiep-vu-web-ke-toan.md#13-kỳ-công--chốt-dữ-liệu-fr-web-period) |
| `E-V21.4` App Quản lý | Chuyển duyệt đơn + theo dõi đội nhóm từ Web sang mobile | 3 tuần | [03](./03-nghiep-vu-app-quan-ly.md) |
| `E-V21.5` App Giám đốc | Dashboard điều hành, approval center, kỳ công, uỷ quyền nhanh, step-up auth | 6 tuần | [04](./04-nghiep-vu-app-giam-doc.md) |
| `E-V21.6` Tách Web Kế toán / Web Giám đốc | Tách route, permission, navigation; giữ chung component | 2 tuần | [05](./05-nghiep-vu-web-ke-toan.md) · [06](./06-nghiep-vu-web-giam-doc.md) |
| `E-V21.7` Step-up authentication | Xác thực bổ sung cho thao tác nhạy cảm trên App và Web (`BR-18`) | 1.5 tuần | [04 mục 11](./04-nghiep-vu-app-giam-doc.md#11-bảo-mật-thao-tác-nhạy-cảm-fr-dir-sec) |
| `E-V21.8` Owner & uỷ quyền | Cấp/chuyển/thu hồi Owner, chặn mất Owner cuối, quyền có thời hạn (`BR-15`) | 2 tuần | [06 mục 11](./06-nghiep-vu-web-giam-doc.md#11-quyền-owner-công-ty-fr-gdw-owner) |
| `E-V21.9` Platform: gói dịch vụ, incident, support access | Package/feature flag, incident management, Support Access Mode | 4 tuần | [07 mục 7–11](./07-nghiep-vu-web-platform-admin.md) |
| `E-V21.10` Dữ liệu hiệu lực theo thời gian | Lịch sử hiệu lực cho phòng ban/chức vụ/quản lý/chính sách (`BR-17`) | 2.5 tuần | [13](./13-mo-hinh-du-lieu.md) |

**Tổng bổ sung: ~29.5 người-tuần.** Cộng vào bảng mục 11, tổng dự án tăng khoảng
**1.5–2 tháng** với đội 6 người.

---

## 13. Tiêu chí nghiệm thu nghiệp vụ (UAT) tối thiểu

Theo v2.1 §12.4. Đây là **cổng chặn go-live**, không phải danh sách mong muốn.

| Nhóm UAT | Tiêu chí |
|---|---|
| **Tenant isolation** | Không tài khoản nào truy cập được dữ liệu tenant khác bằng cách thay ID/URL/API (`BR-09`) |
| **Chấm công** | Chỉ ghi nhận khi đủ điều kiện policy; thời gian tính từ server; **không tạo bản ghi trùng khi retry** (`BR-01`, `BR-19`) |
| **Sinh trắc học** | Không chấp nhận ảnh giả cơ bản; lỗi có mã và hướng dẫn; reset/đổi device có luồng rõ ràng |
| **Đơn từ** | Đúng người duyệt theo hierarchy/workflow; **không tự duyệt đơn của chính mình**; audit đầy đủ (`BR-14`, `BR-16`) |
| **Kế toán** | Tính công **tái lập được**: cùng dữ liệu + cùng policy version cho cùng kết quả. Chốt kỳ khoá sửa trực tiếp (`BR-07`, `BR-12`) |
| **App Giám đốc** | Dashboard đúng scope; approval chống xử lý trùng và chống tự duyệt; thao tác nhạy cảm yêu cầu step-up auth; **mobile và web cho cùng kết quả trạng thái** |
| **Web Giám đốc** | Thay policy/ca/quyền có ngày hiệu lực và audit; cấp scope chính xác; **Owner không thể làm tenant mất Owner cuối** (`BR-15`, `BR-17`) |
| **Platform Admin** | Có health/log/incident để truy vết; support access có mục đích, thời hạn và audit |
| **Báo cáo** | Export đúng bộ lọc, đúng kỳ/version; **số tổng khớp bảng chi tiết** |
| **Anti-fraud** | Mock GPS / replay / thiết bị lạ bị chặn hoặc gắn cờ theo policy, có dữ liệu để đối soát |
| **Khả năng phục hồi** | Backup **đã được restore thử**; lỗi AI/SMS có fallback và error UX rõ ràng |

---

## 14. Danh sách màn hình ưu tiên cho đội UI/Dev

| Ưu tiên | Màn hình |
|---|---|
| **P0 — bắt buộc** | Login, setup face, Home chấm công, tạo đơn, lịch sử, bảng công tháng, duyệt đơn quản lý, **Dashboard + Approval Center App Giám đốc**, nhân sự web, bảng công web, cấu hình giờ/ca, tenant admin |
| **P1 — sau MVP** | Bảng công tuần/năm, lịch ca, phân ca hàng loạt, workflow nhiều cấp, geofence nhiều văn phòng, audit UI, incident UI |
| **P2 — mở rộng** | Offline, widget nhanh, BLE/Wi-Fi anti-fraud, payroll đầy đủ, SSO/SCIM, advanced BI |
