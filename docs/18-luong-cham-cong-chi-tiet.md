# 18 — Một lượt chấm công đi qua những đâu
...
> [15 — Hợp đồng API](./15-hop-dong-api.md) ·
> [17 — Cơ chế chấm công mặt & vân tay](./17-cach-hoat-dong-cham-cong-mat-va-van-tay.md) ·
...
Tài liệu `15` mô tả *hợp đồng* giữa từng cặp thành phần. Tài liệu `17` mô tả *cơ
...
| `X-Body-Sha256` | 401 `AUTH_SIGNATURE_INVALID` | Ràng buộc **nội dung ảnh và toạ độ GPS**. Không có nó thì kẻ chặn được request đã ký giữa đường tráo được ảnh sang người khác — xem [08 mục 1.2.1](./15-hop-dong-api.md#121-x-body-sha256--ràng-buộc-nội-dung-của-request-multipart) |
...
[08 mục 1.2](./15-hop-dong-api.md#12-header-chuẩn).
...
if (result.liveness?.actionVerified !== true)      → FACE_LIVENESS_FAILED
if (bestScore < matchThreshold)                   → FACE_NOT_MATCHED
```

> ⚠ **Phải là `!== true`, không phải `=== false`.** `action_verified` là **ba
> trạng thái**: `true` (đã xác minh), `false` (sai động tác), `null` (không đo
> được — thiếu landmark, mặt nghiêng quá). Viết `=== false` sẽ cho `null` đi
> qua, tức là bất kỳ ảnh nào mà mô hình không đo nổi động tác đều được coi là
> hợp lệ — đúng kịch bản chấm hộ bằng ảnh in mà `AF-05` sinh ra để chặn.
> Code thật đã đúng; câu lệnh trong tài liệu trước đây mới là chỗ sai.
...
> ⚠ Ngưỡng mặc định `0.45` / `0.70` **chưa hiệu chỉnh**. `docs/11` mục 6.4 yêu
...
```jsonc
{
  ...
  "insideGeofence": true,
  "photoUrl": "https://storage.googleapis.com/...?X-Goog-Signature=...",  // hết hạn sau 5 phút
  "fraudScore": 0,
  "flags": []
}
```
...
Ảnh lưu trên Cloud Storage for Firebase được mã hoá at-rest và chỉ truy cập qua signed URL hết hạn
...
[docs/17](./17-cach-hoat-dong-cham-cong-mat-va-van-tay.md): Apple và Google không
