# 11 — Kiến trúc hệ thống & Technology Stack

> **Nguồn chuẩn:** Tài liệu nghiệp vụ SmartFace **v2.1**, Chương I.4 và Chương XI.
> Đây là tài liệu **chính thức để thi công**. Mọi lệch pha giữa code và tài liệu
> này phải được ghi nhận bằng một ADR mới.
>
> Phần công nghệ kế thừa định hướng v2.0 và điều chỉnh theo **sáu phân hệ giao
> diện**, trong đó ba trải nghiệm mobile (Nhân viên / Quản lý / Giám đốc) dùng
> chung **một Flutter codebase** và mở module theo quyền. Đây là đề xuất kỹ
> thuật, có thể thay đổi theo năng lực đội ngũ, nhưng **phải giữ nguyên các
> nguyên tắc bảo mật, multi-tenant và tách AI Server**.
...
```
 ...
 └───────────┬─────────────┘          └───────────────┬──────────────┘
  ...
  │ FastAPI    │ │  SQL    │ │+BullMQ│ │Firebase Stor.│
...
Bảng tổng hợp theo v2.1 Chương XI, bổ sung phiên bản đề xuất và ghi chú thi công.

| Thành phần | Công nghệ | Phiên bản đề xuất | Ghi chú thi công |
|---|---|---|---|
| **App Nhân viên / Quản lý / Giám đốc** | Flutter (Dart) | Flutter 3.24+ | **Một codebase** iOS + Android, mở module theo `role + permission + scope`. Bloc/Riverpod · Dio · local_auth · geolocator · FCM · **step-up auth** cho thao tác executive nhạy cảm |
| **Web Kế toán** | ReactJS + TypeScript | React 18/19 | Ant Design/MUI · TanStack Query · bảng dữ liệu lớn và export |
| **Web Giám đốc** | ReactJS + TypeScript | React 18/19 | Dùng chung monorepo/components với Web Kế toán, **route và permission tách rõ** |
| **Web Platform Admin** | ReactJS + TypeScript | React 18/19 | **Nên tách deployment/domain riêng** để tăng bảo mật cho admin platform |
...
| Lưu trữ ảnh/file | Cloud Storage for Firebase (bucket GCS) | — | Dùng chung service account với Firebase Auth. Mã hoá at-rest mặc định + lifecycle tự xoá. Truy cập qua signed URL V4, TTL ≤ 5 phút |
...
### 4.1. Gợi ý danh sách module Backend (v2.1 §11.1)

```
auth · tenant · organization · employee · role-permission · device · biometric
attendance · shift · shift-assignment · request · approval-workflow
leave · overtime · timesheet · period-close · report · notification
audit · anti-fraud · support · subscription · incident · ai-gateway
```

So với bản v1.0, v2.1 bổ sung các module: **`role-permission`** (RBAC + scope),
**`approval-workflow`** (luồng duyệt cấu hình được), **`period-close`** (state
machine kỳ công), **`support`** (Support Access Mode), **`subscription`** (gói
dịch vụ/feature flags) và **`incident`** (quản lý sự cố).

> ### MVP hạ tầng
>
> **Không bắt buộc Kubernetes/ELK ngay từ ngày đầu** nếu quy mô nhỏ. Có thể chạy
> Docker Compose hoặc managed services — xem
> [22 — Hướng dẫn deploy VPS](./22-huong-dan-deploy-vps.md). Nhưng **phải thiết
> kế đúng service boundary và observability ngay từ đầu**, để khi nâng cấp hạ
> tầng không phải viết lại nghiệp vụ.
...
| `storage` | Upload/download Firebase Storage, signed URL, lifecycle | — |
...
Chi tiết payload xem [15 — Hợp đồng API](./15-hop-dong-api.md).
...
**Tiếp theo:** [12 — Lộ trình triển khai & Nghiệm thu](./12-lo-trinh-va-nghiem-thu.md)
