# 10 — Yêu cầu phi chức năng & Bảo vệ dữ liệu
...
| Bảng `AttendanceLog` quá lớn | Partition theo tháng (`13-mo-hinh-du-lieu.md` mục 4.1) |
...
> [server-backend-smart/docs/storage-lifecycle.md](../server-backend-smart/docs/storage-lifecycle.md).
...
## 11. Chính sách lưu trữ dữ liệu (v2.1 §10.1)

| Loại dữ liệu | Nguyên tắc |
|---|---|
| **Attendance raw** | Giữ đủ để đối soát theo thời hạn pháp lý / chính sách doanh nghiệp. **Không xoá theo yêu cầu miệng** — đây là chứng từ tính công |
| **Ảnh chấm công** | **Không bắt buộc lưu mãi.** Cấu hình retention theo tenant/gói; có thể **chỉ lưu trường hợp nghi vấn** |
| **Face embedding** | Mã hoá, hạn chế truy cập; xoá/ẩn danh theo quy trình nghỉ việc hoặc yêu cầu hợp lệ |
| **Audit log** | Thời hạn **dài hơn** dữ liệu giao diện thông thường; hạn chế quyền xoá (`BR-08`) |
| **File đơn từ** | Retention theo loại chứng từ. **Tài liệu y tế cần quyền truy cập hạn chế hơn** |

> **"Lưu ảnh chấm công mãi mãi" là lựa chọn mặc định tệ nhất.** Nó vừa tốn chi
> phí lưu trữ tăng tuyến tính, vừa mở rộng bề mặt rủi ro pháp lý theo Nghị định
> 13/2023/NĐ-CP. Cấu hình mặc định nên là: **giữ ảnh N ngày, sau đó chỉ giữ
> ảnh của các lượt bị gắn cờ nghi vấn**.

---

## 12. Quyền riêng tư & bảo vệ dữ liệu (v2.1 §10)

| Mã | Yêu cầu |
|---|---|
| `NFR-PRIV-01` | **Data minimization** — chỉ thu thập dữ liệu thực sự cần cho nghiệp vụ chấm công |
| `NFR-PRIV-02` | **Platform Admin không mặc định xem raw biometric.** Truy cập phải có mục đích, quyền nâng cao và audit |
| `NFR-PRIV-03` | **Support access có mục đích và thời hạn**, tự hết hạn khi kết thúc phiên — xem [07 mục 11](./07-nghiep-vu-web-platform-admin.md#11-chế-độ-hỗ-trợ-tenant-có-kiểm-soát-fr-adm-sup) |
| `NFR-PRIV-04` | **Không lưu vân tay thực** — chỉ khoá/attestation gắn với thiết bị (`BR-05`) |
| `NFR-PRIV-05` | Nhân viên xem được dữ liệu sinh trắc học của mình đang được lưu gì, và yêu cầu xoá theo quy trình |
| `NFR-PRIV-06` | Ảnh khuôn mặt thật **không dùng cho debug tuỳ tiện**; sample phải masking/anonymization |

---

## 13. Khả năng kiểm toán (v2.1 §10)

| Mã | Yêu cầu |
|---|---|
| `NFR-AUD-01` | **Audit log bất biến** cho: phân quyền, chính sách, chỉnh công, chốt kỳ, support access |
| `NFR-AUD-02` | Mỗi request có **correlation ID** xuyên Backend → AI Server → Queue → Log |
| `NFR-AUD-03` | Mọi thao tác nhạy cảm ghi **before/after + actor + reason + timestamp** |
| `NFR-AUD-04` | Audit log xuất được cho kiểm toán nội bộ, không sửa/xoá được kể cả bởi Super Admin |

---

**Tiếp theo:** [11 — Kiến trúc & Technology Stack](./11-kien-truc-va-technology-stack.md)
