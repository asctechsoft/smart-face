...
// (tên miền + email + mật khẩu tạm). Xem docs/01 mục 11 và docs/19.
```
...
## 5. Mở rộng schema cần có theo v2.1

> ⚠ **Phần này CHƯA có trong `schema.prisma`.** Mục 3 ở trên mô tả schema **đang
> chạy**; mục này liệt kê những gì phải thêm để đáp ứng chuẩn v2.1. Đừng nhầm
> hai phần.

### 5.1. RBAC + Data Scope (`E-V21.1`)

Hiện tại: `SystemRole[]` + `managedDepartmentIds: String[]` trên `Employee`.
Mô hình này đủ cho một mức scope (`DEPARTMENT`) và không biểu diễn được quyền
theo module hay quyền có thời hạn.

```prisma
enum ScopeLevel { SELF TEAM DEPARTMENT BRANCH COMPANY PLATFORM }

model Role {                       // vai tro tuy bien theo tenant
  id           String   @id @default(uuid())
  companyId    String?              // null = role he thong dung chung
  code         String               // "BRANCH_DIRECTOR", "HR_VIEWER"
  name         String
...
model Permission {                  // danh muc quyen module
  id     String  @id @default(uuid())
  code   String  @unique            // "request.approve", "period.approve_lock"
  module String                     // "request", "period", "policy", ...
}
...
model RoleAssignment {              // gan role cho nguoi + SCOPE + THOI HAN
  id          String     @id @default(uuid())
  companyId   String
  employeeId  String
  roleId      String
  scopeLevel  ScopeLevel
  scopeIds    String[]              // id chi nhanh/phong ban/nhom tuong ung
  validFrom   DateTime   @default(now())
  validTo     DateTime?             // null = vo thoi han; co gia tri = uy quyen tam thoi
  grantedById String
  reason      String?
...
**Quy tắc thi công:** `ScopeGuard` đọc `RoleAssignment` còn hiệu lực tại thời
điểm request, hợp nhất các scope, rồi chèn điều kiện vào mọi query (`BR-13`).
Quyền hết hạn **tự mất tác dụng**, không cần job dọn dẹp.

### 5.2. Owner công ty (`E-V21.8`)
...
**Ràng buộc `BR-15`:** trước khi `revokedAt` được ghi, đếm số Owner còn hiệu lực
của tenant. Nếu kết quả bằng 0 thì **từ chối giao dịch**. Kiểm tra này phải nằm
trong cùng transaction, không phải ở tầng ứng dụng.

### 5.3. Workflow versioning & snapshot (`E-V21.2`)

Hiện tại `ApprovalFlow` được đọc **tại thời điểm duyệt**. Theo `BR-16`, đơn đã
gửi phải giữ luồng duyệt **tại thời điểm gửi**.
...
model ApprovalFlowVersion {
  ...
  flowId     String
  version    Int
  steps      Json                   // snapshot bat bien cua cac buoc duyet
  createdAt  DateTime @default(now())
  @@unique([flowId, version])
}

// Them vao model Request:
//   flowVersionId  String?   // chot khi chuyen sang SUBMITTED
//   approvalSnapshot Json?   // danh sach nguoi duyet da giai quyet tai luc gui
```

Bổ sung `fallbackApproverId` và bảng `ApprovalDelegation` (người uỷ quyền, người
nhận, loại duyệt, scope, `validFrom`/`validTo`).

### 5.4. State machine kỳ công (`E-V21.3`)

```prisma
enum PayrollPeriodStatus {
  OPEN
  CALCULATING
  PENDING_APPROVAL      // MOI — Ke toan da gui de nghi chot
  LOCKED
  REOPENED              // MOI — mo lai co ly do va nguoi duyet
}

model PayrollPeriodVersion {        // MOI — moi lan tinh ra mot version
  id            String   @id @default(uuid())
  periodId      String
  version       Int
  calculatedAt  DateTime @default(now())
  calculatedBy  String
  policySnapshot Json               // policy version da dung, phuc vu tai lap ket qua
  summary       Json                // headcount, tong cong, OT, phep, exception
  @@unique([periodId, version])
}

model PeriodTransition {            // MOI — nhat ky chuyen trang thai
  id          String   @id @default(uuid())
  periodId    String
  fromStatus  PayrollPeriodStatus
  toStatus    PayrollPeriodStatus
  actorId     String
  reason      String?               // BAT BUOC voi REOPENED
  affectedScope Json?               // pham vi du lieu bi anh huong khi mo lai
  createdAt   DateTime @default(now())
}
```

Mọi export phải mang `periodId + version` để hai lần xuất cùng một kỳ cho cùng
số liệu (`FR-WEB-PERIOD-05`).

### 5.5. Dữ liệu hiệu lực theo thời gian (`E-V21.10`, `BR-17`)

Phòng ban, chức vụ và quản lý trực tiếp hiện lưu **trạng thái hiện tại** trên
`Employee`. Xem bảng công tháng trước sẽ dùng nhầm cơ cấu hiện tại.
...
model EmployeeAssignmentHistory {
  ...
  branchId          String?
  effectiveFrom     DateTime
  effectiveTo       DateTime?       // null = dang hieu luc
  changedBy         String
  reason            String?
...
Engine tính công và màn bảng công **phải truy vấn theo `effectiveFrom/To`** ứng
với ngày công, không dùng giá trị hiện tại trên `Employee`.

### 5.6. Step-up authentication & audit mở rộng (`E-V21.7`)

```prisma
model StepUpChallenge {
  id          String   @id @default(uuid())
  userId      String
  action      String                // "owner.assign", "period.reopen", ...
  method      String                // "BIOMETRIC" | "OTP" | "MFA"
  verifiedAt  DateTime?
  expiresAt   DateTime
  createdAt   DateTime @default(now())
}

model SupportAccessSession {        // v2.1 §7.12
  id          String    @id @default(uuid())
  adminUserId String
  companyId   String
  ticketRef   String                // BAT BUOC
  purpose     String                // BAT BUOC
  expiresAt   DateTime              // BAT BUOC
  endedAt     DateTime?
  @@index([companyId, expiresAt])
}
```

Bảng `AuditLog` hiện có cần thêm: `correlationId`, `stepUpChallengeId`,
`supportSessionId` để truy vết đủ chuỗi (`NFR-AUD-02`).

### 5.7. Tóm tắt thay đổi

| # | Hạng mục | Bảng thêm/sửa | Business rule |
|---|---|---|---|
| 1 | RBAC + Scope | `Role`, `Permission`, `RolePermission`, `RoleAssignment` | `BR-13` |
| 2 | Owner công ty | `CompanyOwner` | `BR-15` |
| 3 | Workflow snapshot | `ApprovalFlowVersion`, `ApprovalDelegation`, sửa `Request` | `BR-16` |
| 4 | Kỳ công | `PayrollPeriodStatus` (+2 giá trị), `PayrollPeriodVersion`, `PeriodTransition` | `BR-07`, `BR-12` |
| 5 | Hiệu lực theo thời gian | `EmployeeAssignmentHistory` | `BR-17` |
| 6 | Step-up & support | `StepUpChallenge`, `SupportAccessSession`, sửa `AuditLog` | `BR-18`, `BR-08` |


---

**Tiếp theo:** [14 — Sơ đồ quan hệ bảng dữ liệu](./14-so-do-quan-he-bang-du-lieu.md)
