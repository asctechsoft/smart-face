...
- [`docs/05-nghiep-vu-web-ke-toan.md`](../docs/05-nghiep-vu-web-ke-toan.md) — nghiệp vụ
- [`docs/15-hop-dong-api.md`](../docs/15-hop-dong-api.md) + [`docs/16-danh-muc-api-backend.md`](../docs/16-danh-muc-api-backend.md) — hợp đồng API
- [`docs/20-quy-chuan-style-guide.md`](../docs/20-quy-chuan-style-guide.md) — thiết kế

> Ghi chú lịch sử: file README cũ mô tả thư mục này là *employee self-service portal*.
> Theo `docs/README.md` ("Thi công Web: `05` → `06` → `07`") và `ADR-03`, `web-smart` phục vụ
> **Web Kế toán + Web Giám đốc** (theo v2.1 cần tách route/permission — xem `docs/05` và `docs/06`);
...
├── components/
│   ├── ui/            # THƯ VIỆN COMPONENT DÙNG CHUNG — docs/20 mục 11
...
| Đường dẫn | Màn hình | Mã yêu cầu (`docs/05`) |
|---|---|---|
...
| `/fraud` | Cảnh báo gian lận | `docs/09` mục 7 |
...
tài khoản sẵn, xem banner đầu [`docs/05`](../docs/05-nghiep-vu-web-ke-toan.md).

---

## Thư viện component dùng chung

Dựng theo [`docs/20` mục 11](../docs/20-quy-chuan-style-guide.md), mỗi component đủ **5 trạng thái**
...
| Hạng mục | Đo được | Yêu cầu docs/20 |
|---|---|---|
...
toàn bộ bằng `RolesGuard` + `ScopeGuard` (`docs/05` mục 12.2). Đừng bao giờ dựa vào nó như một chốt bảo mật.
...
(`docs/06` mục 6.5). ESLint chặn `Date.toLocaleString`.
...
Mọi màu lấy từ token ngữ nghĩa trong `src/styles/tokens.css`, chép nguyên văn từ `docs/20` mục 1–2.
...
| Nút chính (amber) | Hover **SÁNG LÊN** (`amber-400`), không tối đi. Antd mặc định tối → 3.02:1, trượt AA (`docs/20` mục 0.1) |
...
| Xuất Excel | Luôn qua job ở Backend (`docs/05` mục 7.4). Không dựng file ở client cho bảng công/bảng lương |
...
Những mục trong `docs/05` chưa thi công ở bản này, xếp theo thứ tự ưu tiên:
...
| Chế độ tối | `docs/20` mục 12 | Token đã sẵn sàng, chỉ cần công tắc và lưu lựa chọn của người dùng |
...
Thư viện dựng từ **`docs/20` mục 11**, không phải đọc trực tiếp từ Figma. Lý do và việc cần làm:
...
| Hai file Figma khác nhau | `docs/20` mục 17 ghi file key `COLgmXH63JZ2274UQhozCx`. Link được gửi trỏ tới `YGR1jGWuIykwhpEC9VYykY` — **file khác** |
| `docs/20` mới hơn Figma | Mục 15 của tài liệu liệt kê **8 điểm Figma cần sửa để khớp lại** (badge sai WCAG, hai hệ nút, màu viền trùng lặp…). Nên khi hai nguồn lệch nhau, `docs/20` thắng |
